					...
					for(var i in tmpguan){
						...
						for(var j=0;j<data.map[Row.rows[tmpguan[i][1]].orderNo].length;j++){
		        	  	...
		        	  	if(Row.rows[tmpguan[i][1]].orderStatus=='090001'){
		        	  		...
		        	  		content+="<button type='button' class='affirm_indent' onclick=cancelOrder(\'"+Row.rows[tmpguan[i][1]].orderNo+"\',\'"+Row.rows[tmpguan[i][1]].orderType+"\')>取消订单</button>";
		        	  		content+="</div>";
		        			...
		        			content+="<div class='row text-bar-right'>";
		        		  	...
		        		  	content+="<button type='button' class='affirm_indent' onclick=operatorOrder(\'"+Row.rows[tmpguan[i][1]].orderNo+"\',this)>确认付款</button>";
		        		  	content+="</div>";
...
function cancelOrder(orderNo,orderType){
	layer.open({
		title: '提示信息'
		,content:"是否取消已选中的订单?<br/><input class='cause' placeholder='请输入取消的原因'>"
		,btn:['确认','取消']
