...
$(function(){
	...
	$(".sheet").on("click",".input_price",function(){
		var price = $(this).children(".fs14").text();
		var str = $(this).children(".fs12").text();
		if(str=="无折扣"){
			var discount = 10;
		}else{
			var discount = str.substring(0,str.length-1);
		}
		var compile_modle = '<div><label>商品价格<input type="number" class="price_update" value='+price+'></label><span></span></div>';
		compile_modle = compile_modle + '<div><label>商品折扣<input type="number" min="0" max="10" class="discount_update" value='+discount+'></label><span></span></div>';
		...
		layer.open({
			...
			yes:function(){
				if($('.discount_update').val()>10||$('.discount_update').val()<0){
					...
					var goodsDiscount = $('.discount_update').val();//折扣
	...
	function demo(curr){
		$.ajax({
			...
			data:{pageNo:curr||1,orderStatus:'090004'},
			beforeSend:function(){
				$(".sheet").html("获取数据中");
			},
			success:function(data){
					...
					for(var i in tmpguan){
						...
						for(var num = 1; num < tmpguan[i].length; num++){
					       	...
					       	if(Row.rows[tmpguan[i][1]].orderStatus=='090001'){
					        	content+="<div class='indent_price input_price'><input type='hidden' value="+Row.rows[tmpguan[i][num]].tableKey+"><span class='fs14 vertical-m' style='color:blue;'>"+Row.rows[tmpguan[i][num]].goodsPrice+"</span><br/>";
						...
						if(Row.rows.orderStatus=='090003'||Row.rows.orderStatus=='090004'||Row.rows.orderStatus=='090005'){
					   	...
					   	if(Row.rows[tmpguan[i][1]].orderStatus=='090001'){
					   		...
					   		content+="<button type='button' class='affirm_indent' onclick=operatorOrder(\'"+Row.rows[tmpguan[i][1]].orderNo+"\',this)>确认发货</button>";
					   	}else if(Row.rows[i].orderStatus=='090005'){
					   		content+="<span>总计：<em>￥"+price+"</em></span>";
					   	}else if(Row.rows[i].orderStatus=='090006'){
					   		content+="<button type='button' class='affirm_indent' onclick=deleteOrder(\'"+Row.rows[tmpguan[i][1]].orderNo+"\')>删除订单</a>"; 
					   	}
...
//订单拆分
function load(val){location.href = "busOrder/getbygood.do?key="+val;}
//取消订单
function cancelOrder(orderNo,orderType){
	layer.open({
		title: '提示信息'
		,content:"是否取消已选中的订单?<br/><input class='cause' placeholder='请输入取消的原因' maxlength='150'>"
		,btn:['确认','取消']
				...
				success:function(data){
					if(data.success){
						...
						layer.msg('操作成功', {
							...
							$(".sheet").parent().load("busOrder/toApplyAll.do");
						});
					}else{
						layer.open({
							title: '错误信息'
	...
	layer.open({
		...
		,yes: function(index, layero){
			...
			$.ajax({
				...
				data:{"orderNo":orderNo,"type":"0"},//增加操作类型，以便于后续扩展
				dataType:"json",
				type:'post',
				success:function(data){
					if(data.success){
